...
export function Reader() {
  ...
  // ✅ Track when user reads an episode (using new tracking system)
  const readCount = useTrackView(story?.id, "read");
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const pct = Math.max(
        0,
        Math.min(100, (window.scrollY / Math.max(total, 1)) * 100),
      );
          ...
          {visiblePages.map((pageNum) => {
            ...
            return (
              <button
                ...
                title={
                  isLocked ? "Bonyeza kununua hadithi" : `Sehemu ya ${pageNum}`
                }
              >
                {pageNum}
              </button>
  ...
  const SimilarStories = () => {
    ...
    return (
      <section
        ...
        aria-label="Hadithi zinazofanana"
      >
  ...
  return (
    <div
      className={cn(
        "min-h-screen w-full",
        light ? "bg-white text-gray-900" : "bg-[#0E0C0D] text-[#F6F0E8]",
      )}
    >
      ...
      >
        <div className="mx-auto flex h-14 max-w-5xl items-center gap-3 px-4 sm:px-6">
          ...
          <Link
            to="/"
            className={cn(
              "font-display text-lg font-black tracking-[0.14em]",
              light ? "text-[#9B1B3B]" : "text-[#F6F0E8]",
            )}
          >
            KISA
          </Link>
      ...
      <div ref={articleRef} className="px-5 pb-40 pt-10 sm:px-8 sm:pt-14">
        ...
        >
          ...
          >
            ...
            Dakika {ep.reading_minutes || 5} za kusoma ·{" "}
          ...
          <div
            className={cn(
              "mt-8 mb-8 h-px w-16",
              light ? "bg-gray-200" : "bg-[#C9A24A]/40",
            )}
            aria-hidden="true"
          />
            ...
            {paragraphs.map((p: string, i: number) => (
              <p
                ...
                className={cn(
                  ...
                  i === 0 &&
                    "first-letter:float-left first-letter:mr-2 first-letter:font-display first-letter:text-[56px] first-letter:font-black first-letter:leading-[0.85]",
                )}
              >
                {p}
              </p>
