...
export function Profile() {
  const { user, logout, readerPrefs, setReaderPrefs } = useKisa();
  ...
  if (!user) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 sm:px-6">
        <EmptyState
          title="Hakuna akaunti iliyoingia"
          body="Ingia ili kuona na kubadilisha taarifa zako."
          ctaLabel="Ingia"
          ctaHref="/ingia" />
  ...
  return (
    <div className="mx-auto max-w-[1000px] px-4 pb-24 pt-10 sm:px-6 lg:px-10">
      ...
      <section
        ...
        className="mt-8 rounded-card border border-line-soft bg-surface p-5 sm:p-7">
        ...
        <div className="flex flex-wrap items-center gap-5">
          ...
          <div className="min-w-0">
            <h2 id="profile-info" className="font-display text-2xl font-bold text-cream">
              {user.name}
            </h2>
            <p className="mt-1 text-sm text-mist">{user.phone}</p>
          ...
          <div className="ml-auto rounded-xl border border-gold/25 bg-[#171112] px-4 py-3">
            ...
            <p className="mt-1 text-xs text-mist">
              {user.premium ? `Inaisha baada ya siku ${user.daysLeft}` : 'Ongeza muda ili kuendelea'}
            </p>
          </div>
        </div>
        <p className="mt-5 text-xs text-dust">Mwanachama tangu {user.joined}</p>
      </section>

      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <section aria-labelledby="reading-prefs" className="rounded-card border border-line-soft bg-surface p-5 sm:p-6">
          <h2 id="reading-prefs" className="font-display text-lg font-bold text-cream">
            Mapendeleo ya kusoma
          </h2>
          ...
          <div className="mt-5">
            ...
            <div className="mt-2 flex items-center gap-3">
              <input
                ...
                onChange={(e) => setReaderPrefs((p) => ({ ...p, fontSize: Number(e.target.value) }))}
                aria-label="Ukubwa wa maandishi"
                className="w-full accent-[#9B1B3B]" />
              
              <span className="w-12 shrink-0 text-right text-sm tabular-nums text-cream">
                {readerPrefs.fontSize}px
              </span>
            </div>
            <p
              className="mt-3 rounded-lg border border-line-soft bg-surface-raised px-4 py-3 font-read text-cream/85"
              style={{ fontSize: `${readerPrefs.fontSize}px`, lineHeight: 1.85 }}>
          ...
          <div className="mt-6">
            ...
            <div className="mt-2 flex gap-2">
              ...
              <button
                key={m}
                onClick={() => setReaderPrefs((p) => ({ ...p, mode: m }))}
                aria-pressed={readerPrefs.mode === m}
                className={cn(
                  'flex-1 rounded-xl border px-4 py-3 text-sm font-semibold transition-colors duration-150 ease-kisa',
                  readerPrefs.mode === m ?
                  'border-gold/60 bg-gold/10 text-gold' :
                  'border-line bg-surface-raised text-mist hover:text-cream'
                )}>
          ...
          <div className="mt-6">
            ...
            <div className="mt-2 flex gap-2">
              ...
              <button
                key={w}
                onClick={() => setReaderPrefs((p) => ({ ...p, width: w }))}
                aria-pressed={readerPrefs.width === w}
                className={cn(
                  'flex-1 rounded-xl border px-4 py-3 text-sm font-semibold transition-colors duration-150 ease-kisa',
                  readerPrefs.width === w ?
                  'border-gold/60 bg-gold/10 text-gold' :
        ...
        <div className="space-y-6">
          <section aria-labelledby="notif-prefs" className="rounded-card border border-line-soft bg-surface p-5 sm:p-6">
            <h2 id="notif-prefs" className="font-display text-lg font-bold text-cream">
              Taarifa
            </h2>
            <ul className="mt-4 space-y-4">
              ...
              <li key={t.key} className="flex items-start justify-between gap-4">
                  ...
                  <button
                  role="switch"
                  aria-checked={notif[t.key]}
                  aria-label={t.label}
                  onClick={() => setNotif((n) => ({ ...n, [t.key]: !n[t.key] }))}
                  className={cn(
                    'relative h-6 w-11 shrink-0 rounded-full transition-colors duration-150 ease-kisa',
                    notif[t.key] ? 'bg-wine' : 'bg-surface-high'
                  )}>
                  
                    <span
                    className={cn(
                      'absolute top-0.5 h-5 w-5 rounded-full bg-cream transition-transform duration-150 ease-kisa',
                      notif[t.key] ? 'translate-x-[22px]' : 'translate-x-0.5'
                    )} />
          ...
          <section aria-labelledby="security" className="rounded-card border border-line-soft bg-surface p-5 sm:p-6">
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="flex flex-col gap-2 border-t border-line-soft pt-4 sm:flex-row">
                <ButtonLink to="/premium" variant="secondary" className="flex-1">
                  Simamia usajili
                </ButtonLink>
                <Button
                  variant="ghost"
                  ...
                  onClick={() => {
                    logout();
                    toast.message('Umetoka kwenye akaunti');
                  }}>
