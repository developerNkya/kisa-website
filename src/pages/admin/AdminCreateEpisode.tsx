...
export function AdminCreateEpisode() {
  ...
  // Store the selected story's price for context
  const [selectedStoryPrice, setSelectedStoryPrice] = useState<number>(1000);
  ...
  async function loadStories() {
    const { data } = await supabase.from('stories').select('id, title, price').order('title');
    if (data) {
      setStories(data);
      if (data.length > 0) {
        setStoryId(data[0].id);
        setSelectedStoryPrice(data[0].price || 1000);
      }
    }
  }

  // 🔄 Smart default for is_free based on story price and episode number
  useEffect(() => {
    ...
    // Rule 1: If story is free (price = 0), all episodes are free
    if (storyPrice === 0) {
      setIsFree(true);
      setSelectedStoryPrice(0);
      return;
    }

    // Rule 2: For premium stories, first 3 episodes are free by default
    if (epNum <= 3) {
      setIsFree(true);
    } else {
      // Rule 3: Episodes > 3 are premium by default
      setIsFree(false);
    }

    setSelectedStoryPrice(storyPrice);
  }, [number, storyId, stories]);
  ...
  const handleStoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (story) {
      setSelectedStoryPrice(story.price || 1000);
    }
  };
  ...
  const epNum = parseInt(number, 10);
  const isStoryFree = selectedStoryPrice === 0;
          ...
          <AdminPanel title="Publishing">
            <div className="space-y-4 p-4 sm:p-5">
              ...
              <div>
                ...
                {/* 🔍 Helper text showing why this is the default */}
                <p className="mt-2 text-xs text-zinc-500">
                  {getAccessHelperText()}
                </p>
                {!isStoryFree && selectedStoryPrice > 0 && (
                  <p className="mt-1 text-xs text-zinc-600">
                    💰 Bei ya hadithi: TSh {selectedStoryPrice.toLocaleString()}
                  </p>
                )}
              </div>
